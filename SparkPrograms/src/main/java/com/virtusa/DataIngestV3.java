 ...
 */
public class DataIngestV3 {
    private static final String FILE_FORMAT = "csv";
    private static final String FILE_PREFIX = "TransData_";
    private static final String NAMENODE_PROPERTY ="fs.defaultFS";
    private static final String NAMENODE ="hdfs://172.31.26.104:8020";
    private static final String EXPECTED_HEADERS[] ={"ID","TransDesc","TransRecordID","Currency","Date of Feed Trade","Max No Of Trades","Approval Date","Approval Name","Risk Status"};
        ...
        if(EXPECTED_HEADERS.length == actualHeaders.length) {
            for (int i = 0; i<EXPECTED_HEADERS.length; i++){
                if(!EXPECTED_HEADERS[i].equals(actualHeaders[i])) {
                    System.err.println("Expected header name not matching with actual header in the file " + source + "Header Mismatch "+EXPECTED_HEADERS[i]+" and "+actualHeaders[i]);
                    return false;
                }
            }
        }else {

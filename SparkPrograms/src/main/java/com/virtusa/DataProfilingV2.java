...
public class DataProfilingV2 {
    ...
    public static void main(String args[]){
        ...
        // overall profiling
        DataFrame profilingDF = sqlContext.sql(idQuery+transDescQuery+transRecordIDQuery+currencyQuery+dateOfFeedTradeQuery+maxNoOfTradesQuery+approvalDateQuery+approvalTimeQuery+riskStatusQuery);
        profilingDF.show();

        // Currency level profiling
        DataFrame currencyDF = sqlContext.sql("select "+timestamp+" astimestamp,Currency,count(*) as count,sum(`Max no of Trades`) as TotalTrades,avg(`Max no of Trades`) AvgTrade,max(`Max no of Trades`) MaxTrade,min(`Max no of Trades`) MinTrade from customer group by Currency");
        currencyDF.show();

...
public class DataIngestV5 {
    private static final String FILE_FORMAT = "csv";
    private static final String FILE_PREFIX = "TransData_";
    ...
    final static Logger logger = Logger.getLogger(DataIngestV5.class);


    public static void main(String args[]) throws IOException {
        if(args.length!=4){
            logger.error("Please provide Four arguments source(file/dir) targetdir ArchivalDir ArchivalFailureDir");
            logger.error("Target dir is in HDFS and others are LFS");
            logger.error("USAGE: java DataIngestV3 /data/file1.csv /rawdata/20160928 /archival/success /archival/failure");
            System.exit(1);
        }
    ...
    public static void writeFileToHDFS(String source,String target,Configuration conf,String successDir, String failureDir) throws IOException {
        ...
        if (fileCheck(source)) {
            ...
            logger.info("Number of lines in the file: "+count);
            try {
                ...
                logger.error("Error occured while pushing the file " + source + " to HDFS "+e.getMessage());
                return;
            }
            logger.info(source + " copied to HDFS Successfully");
            moveFile(source,successDir);
        }else {
            logger.error(source + " File is not satisfying the format or filename");
        ...
        if(EXPECTED_HEADERS.length == actualHeaders.length) {
            for (int i = 0; i<EXPECTED_HEADERS.length; i++){
                if(!EXPECTED_HEADERS[i].equals(actualHeaders[i]))
                    ...
                    logger.error("Expected header name not matching with actual header in the file "+source);
            }
        }else {
            logger.error("Expected number of headers not equal to Actual headers in the file "+source);
        }*/
        return true;
    }
